// app/admin/music/list/page.tsx - Music Management List
'use client';

import { useState } from 'react';
import useSWR from 'swr';
import Link from 'next/link';
import { FaEdit, FaTrash, FaPlus, FaSearch, FaFilter, FaMusic, FaEye, FaEyeSlash, FaYoutube } from 'react-icons/fa';
...
const categories = ['All', 'Single', 'Album', 'Live Performance', 'Cover', 'Reel/Short', 'Worship', 'Music Video'];

export default function MusicListPage() {
  const { data: music, mutate, isLoading } = useSWR<Music[]>('/api/music?limit=100', fetcher);
  ...
  const deleteMusic = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        mutate(music?.filter(m => m.id !== id), false);
      } else {
  ...
  return (
    <div className="max-w-7xl mx-auto p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900">Music</h1>
          <p className="text-slate-500 mt-1">Manage all your music and videos</p>
        </div>
        <Link
          href="/admin/music"
          className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition"
        >
          <FaPlus /> Add Music
        </Link>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-10 pr-8 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white"
            >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900">YouTube</th>
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img
                          src={item.thumbnail || getThumbnailUrl(item.youtubeId)}
                          alt=""
                          className="w-12 h-12 rounded-lg object-cover"
                        />
                        <div className="absolute inset-0 flex items-center justify-center">
                          <FaYoutube className="text-white text-lg drop-shadow-lg" />
                        </div>
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4 text-sm">
                    <a 
                      href={item.youtubeUrl || `https://youtube.com/watch?v=${item.youtubeId}`}
                      target="_blank"
        ...
        {filteredMusic?.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-slate-900 mb-2">No music found</h3>
            <p className="text-slate-500 mb-6">Add your first music track to get started</p>
            <Link
              href="/admin/music"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition"
            >
      ...
      {music && (
        <div className="mt-6 flex items-center justify-between text-sm text-slate-500">
          <p>Showing {filteredMusic?.length || 0} of {music.length} tracks</p>
          <div className="flex gap-4">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-green-500" />
              {music.filter(m => m.published).length} Published
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-slate-400" />
              {music.filter(m => !m.published).length} Drafts
            </span>
          </div>
        </div>
      )}
